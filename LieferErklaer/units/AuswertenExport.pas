...
begin
  ...
  if not Initialized then
    exit;
  //Lies die Tage, die eine Lief.-Erkl�r. mindestens noch gelten muss
  ...
  //Fuege Teile von Lieferanten mit g�ltiger Erkl�rung "alle Teile" ein
  LocalQry.LeklAlleTeileInTmpTabelle(minRestGueltigkeit);

  //Fuege Teile von Lieferanten mit g�ltiger Erkl�rung "einige Teile" ein
  LocalQry.LeklEinigeTeileInTmpTabelle(minRestGueltigkeit);

  //Leere Zwischentabelle
  LocalQry.RunExecSQLQuery('delete from tmp_anz_xxx_je_teil;');

  //Anzahl der Lieferanten mit g�ltiger Erklaerung je Teil in tmp Tabelle
  LocalQry.UpdateTmpAnzErklaerungenJeTeil;

  //Anzahl der Lieferanten mit g�ltiger Erklaerung je Teil
  //in Tabelle Teile auf 0 setzen
  LocalQry.RunExecSQLQuery('UPDATE Teile SET n_LPfk= 0');

  //Anzahl der Lieferanten mit g�ltiger Erklaerung je Teil in Tabelle Teile
